...
class RateLimited(OpenDotaError):
    """Сервер устойчиво отказывает по темпу.

    Отделено от исчерпания квоты: это состояние временное, конкретный запрос
    можно повторить позже, и оно не должно ни прерывать выгрузку, ни помечать
    данные недоступными.
    """
        ...
        for attempt in range(retries + 1):
            ...
            if response.status_code == 429:
                ...
                if attempt == retries:
                    ...
                    raise RateLimited(f"сервер устойчиво отвечает 429 на {key}")
                continue
